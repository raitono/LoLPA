...
const parseAndLoad = async (version) => {
	...
	batches.push(fs.readFileAsync(tempFilePath + 'item.json')
		.then((data) => {
			...
			return Promise.all(itemBatch.map(request)).then(() => {
				debug('Items added');
				return request({
					method: 'POST',
					uri: webServer.URLs.ItemStat.resetExists(),
					json: true,
				}).then(() => {
					return Promise.all(itemStatBatch.map(request)).then(() => {
						return request({
							method: 'POST',
							uri: webServer.URLs.ItemStat.deleteMissing(),
							json: true,
						}).then(() => {
									...
									return Promise.all(tagBatch.map(request), itemMapXrefBatch.map(request)).then(() => {
										...
										return request({
											...
											return request({
												...
												return Promise.all(tagXrefBatch.map(request)).then(() => {
